...
void loop()
{
  while (1){
    if (analogRead(A0) > SIGNAL_THRESHOLD){ 
      ...
      // Wait for ~8us this is using analogread as a very short delay time.
      analogRead(A3);
...
void get_time() 
{
  ...
  if (muon_count > 0.) {
      count_average   = muon_count / ((interrupt_timer - start_time - total_deadtime) / 1000.);
      count_std       = sqrt(muon_count) / ((interrupt_timer - start_time - total_deadtime) / 1000.);}
  else {
      count_average   = 0;
      count_std       = 0;}
...
{
  float voltage = 0;
  for (int i = 0; i < (sizeof(calibration)/sizeof(float)); i++) {
